...
# [REMOVIDO 2026-08-13] _N_MATERIALS/_MAGNET_ID/_MU_BY_ID + as funções puras de
# parsing/geometria abaixo (_parse_solution .. _assign_mesh_to_grid) movidas pra
# src/data_gen/parsers/ans_parsing.py -- este arquivo tem `import femm` (COM),
# necessário só pras funções que abrem o FEMM de verdade (generate_mesh_sample,
# save_ans_gzip_sample, _query_grid_pointvalues); importar as funções puras
# arrastava esse import junto mesmo pra quem só quer parsear o .ans em disco
# (ex: pipeline femm_mesh_v2, que roda sem FEMM aberto). Reimportadas abaixo,
# pra generate_mesh_sample continuar funcionando sem duplicar código -- e pro
# `import matplotlib.tri as mtri` sumir daqui (só usado por _assign_mesh_to_grid,
# agora em ans_parsing.py).
...
from src.data_gen.parsers.ans_parsing import (
    _N_MATERIALS, _MAGNET_ID, _MU_BY_ID,
    ...
    _wrap_edge_pairs, _build_bidirectional_edge_attrs,
    _grid_polar_xy, _assign_mesh_to_grid,
)


# [REMOVIDO 2026-08-13] leitura do .ans + geometria da malha + grade H×W
# (_parse_solution .. _assign_mesh_to_grid) -- movidas pra
# src/data_gen/parsers/ans_parsing.py (funções 100% puras, sem `import femm`),
# reimportadas acima. generate_mesh_sample abaixo continua chamando-as, só que
# agora como orquestrador (abre FEMM via COM, depois chama essas funções puras
# sobre o resultado). Ver docstring de ans_parsing.py para o motivo completo.
# # ---------------------------------------------------------------------------
